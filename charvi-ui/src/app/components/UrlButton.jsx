...
function UrlButton({nodeId,urlId}){
    ...
    return(
        <div>
            <Popover>
                <PopoverTrigger>
                    <input maxLength={20} defaultValue={txturl} readOnly className="outline-none" placeholder='Tittle...' style={{ width: `${Math.max(48, Math.min((txturl.length + 1) * 8, 200))}px` }} type='text'></input>
                </PopoverTrigger>
                <PopoverContent className="text-white w-80 bg-[#303030] border border-[#3C3D37] drop-shadow-md">
                    <div className="grid gap-4">
                        <div className="space-y-2">
                            <h4 className="leading-none text-[#21c063] font-semibold">Tittle</h4>
