  ...
  return (
    <div className="flex justify-center relative drop-shadow-xl">
      <div className="">
        <input
          maxLength={20}
          onBlur={() => addOrmMenuObj(nodeId, menutittle, menuId)}
          onChange={(e) => setMenutittle(e.target.value)}
          className="outline-none"
          placeholder="Tittle..."
          style={{
            width: `${Math.max(48, Math.min((menutittle.length + 1) * 9, 200))}px`,
          }}
          type="text"
        ></input>
      </div>
        ...
        <div className="p-[15px] w-full">
          <div className="flex justify-end m-[2px]">
            <button
              tabIndex={-1}
              onClick={addRow}
              className="cursor-pointer p-[5px] bg-white text-black font-normal rounded"
            >
              Add Row
            </button>
          </div>
                ...
                <div className="flex space-x-1 rounded hover:bg-zinc-600 p-[7px]">
                  ...
                  <div>
                    <input
                      value={row.tittle}
                      onChange={(e) =>
                        updateRow(index, "tittle", e.target.value)
                      }
                      maxLength={24}
                      className="text-white font-normal outline-none"
                      placeholder={`Row ${index + 1} tittle...`}
                      type="text"
                    ></input>
                    <TextareaAutosize
                      ...
                      maxLength={72}
                      className="no-focus-border border-none resize-none text-muted-foreground w-full outline-none font-normal"
