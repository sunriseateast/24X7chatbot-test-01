...
function ActionButton({ nodeId, buttonId }) {
  ...
  const [buttonhandleid, setButtonhandleid] = useState(uuid());
  ...
  return (
    <>
      ...
      <div>
        <input
          maxLength={20}
          onChange={(e) => setActiontittle(e.target.value)}
          onBlur={() => addActionObj(nodeId, actiontittle, buttonId)}
          className="outline-none"
          placeholder="Tittle..."
          style={{
            width: `${Math.max(48, Math.min((actiontittle.length + 1) * 9, 200))}px`,
          }}
          type="text"
        ></input>
      </div>
