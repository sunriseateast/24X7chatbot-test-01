...
const Newnode=React.memo(function Newnode({selected,id,data}){
    ...
    const [handleid,setHandleid]=useState(  // To create random id
    ...
    useEffect(() => {
        if(data.addbutton.tittle=='url'){ //if url is provide in data it checks first action is present or not in bowl
            if(values.some(item=>item.tittle=='action')){
                Toast('Whatsapp Limitaions','Interactive + Url Button not allowed','Int+Url')
            }
            else if(values.some(item=>item.tittle=='list')){
                Toast('Whatsapp Limitaions','List + Url Button not allowed','Lst+Url')
            }
            else{
                if(values.some(item => item.tittle=='media') &&  data.addbutton.tittle=='url' && values.length < 2){
                     setValues(prev=>{
        ...
        if(data.addbutton.tittle=='action'){ // if action is provide in data it check url is present or not in bowl
            if(values.some(item=>item.tittle=='url')){
               Toast("Whatsapp Limitaions",'Url + Interactive Button not allowed','Url+Int')
            }
            else if(values.some(item=>item.tittle=='list')){
               Toast("Whatsapp Limitaions","List + Interactive Button not allowed","Lst+Int")
            }
            else{
                if(values.some(item=>item.tittle=='media') && data.addbutton.tittle=='action' && values.length < 4){
                     setValues(prev=>{
                        return [...prev, data.addbutton];
                    })
                }
                   ...
                   Toast("Whatsapp Limitaions","Only 3 Intractive Buttons are allowed","Int")
                }
            }
        }
        ...
        if(data.addbutton.tittle=='list'){
            if(values.some( item => item.tittle =='url')){
               Toast("Whatsapp Limitaions","Url + List Button not allowed","Url+Lst")
            }
            else if(values.some( item => item.tittle =='action')){
                Toast("Whatsapp Limitaions","Interactive + List Button not allowed","Int+Lst")
            }
            else{
                if(values.some(item=>item.tittle=='media') && data.addbutton.tittle=='list' && values.length < 2){
                     setValues(prev=>{
                        return [...prev, data.addbutton];
                    })
                }
    ...
    const remove=(idToRemove,titleToRemove)=>{
        updateNodeInternals(id)
        setValues(prev =>
            prev.filter(item => !(item.id === idToRemove && item.tittle === titleToRemove))
        )
    ...
    return(
        ...
        rounded-xl`}>
            <div>
                <Handle isConnectableStart={false} isConnectable={!live} className='p-[4px]' type="target" position="left" id={handleid}/>
                <div onClick={deleteNode} className="cursor-pointer p-[7px] rounded-xl absolute top-0 right-0 grid justify-items-end text-zinc-600 hover:text-red-500">
                    <div>
                        <Trash/>
                    </div>
                </div>
                ...
                <div className="text-[#21c063] font-semibold">
                 {values
                    ...
                    .map((item, index) => (
                        <div key={item.id} className="relative cursor-pointer w-full flex items-center justify-center gap-x-[8px] p-[8px] border-t border-zinc-600">
                            <div onClick={() => remove(item.id, 'action')} className="text-zinc-600 absolute left-1">
                                <Cancel />
                            </div>
